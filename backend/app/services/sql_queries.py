...
JOIN SA1010 A1
       ...
       AND A1.D_E_L_E_T_ = ' '
JOIN SB1010 B1
        ON B1.B1_FILIAL = C6.C6_FILIAL
       AND B1.B1_COD = C6.C6_PRODUTO
       AND B1.D_E_L_E_T_ = ' '
LEFT JOIN SA3010 A3
        ON A3.A3_FILIAL = C5.C5_FILIAL
       AND A3.A3_COD = C5.C5_VEND1
       AND A3.D_E_L_E_T_ = ' '
LEFT JOIN (
    SELECT B2_FILIAL, B2_COD, B2_LOCAL, SUM(B2_QATU) AS B2_QATU
    FROM SB2010
    WHERE D_E_L_E_T_ = ' '
    GROUP BY B2_FILIAL, B2_COD, B2_LOCAL
) B2 ON B2.B2_FILIAL = C6.C6_FILIAL
     AND B2.B2_COD    = C6.C6_PRODUTO
     AND B2.B2_LOCAL  = C6.C6_LOCAL
LEFT JOIN (
    SELECT E4_CODIGO, MAX(E4_DESCRI) AS E4_DESCRI
    FROM SE4010
    WHERE D_E_L_E_T_ = ' '
    GROUP BY E4_CODIGO
) E4 ON E4.E4_CODIGO = C5.C5_CONDPAG
WHERE C5.D_E_L_E_T_ = ' '
  AND C5.C5_FILIAL = '02'
